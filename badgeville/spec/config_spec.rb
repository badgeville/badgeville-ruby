...
module Badgeville
  describe Config do
    ...
    context "where the request site is empty, but the api key is valid"
    before do
      @empty_string = ''
    end
    ...
    context "where the request site is not passed in (is nil), but the api key is valid"
    it "should raise an ArgumentError with an error message" do
      lambda { Config.conf(:api_key => @my_api_key) }.should raise_error(ArgumentError)
    end


    context "where the API Key is empty, but the request site is valid"
    it "should raise an ArgumentError with an error message" do
      lambda { Config.conf(:site => @request_site, :api_key => @empty_string) }.should raise_error(ArgumentError)
    end


    context "where the API Key is not passed in (is nil), but the request site is valid"
    it "should raise an ArgumentError with an error message" do
      lambda { Config.conf(:site => @request_site) }.should raise_error(ArgumentError)
    end


    context "where conf is called with no parameters"
  ...
  describe Config do
    context "where the request site has no scheme (i.e. no 'http')"
      before do
