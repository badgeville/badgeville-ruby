  ...
  describe "README use case example set" do
    before(:all) do
      ...
      @player_created = @new_player.save
       ...
       debugger
      # # Basic README: Find the newly created player to update their email address
      @player_found_by_id               = Player.find( @new_player.id )
      @player_found_by_id.display_name  = "New Player Name"
      @player_updated                   = @player_found_by_id.save
      @player_w_updated_email           = Player.find( @new_player.id )
...
#{"command":"count","comparator":1,"where":\{"verb":"verb","player_id":"%player_id"\}\}\
      @new_reward_defn = RewardDefinition.new( :site_id          => @new_site.id,
                                               :name             => 'Comment Rockstar',
                                               :reward_template  => '{"message":"Congrats, you are a Comment Rockstar!"}',
                                               #:components       => '{"command":"count","comparator":1,"where":{"verb":"comment","player_id":"%player_id"}}',
                                               :active           => true

                                    )
      @rd_created = @new_reward_defn.save
    ...
    # # UPDATE Player
    it "should have updated the newly created player", :affects_bv_server => true do
       @player_updated.should == true
    end

    it "should have found the newly created player by ID to update their email address", :affects_bv_server => true do
       @player_w_updated_email.display_name.should == "New Player Name"
    end
    ...
    it "should have added 3 points to @new_player", :affects_bv_server => true do
      @updated_player.points_all.should == 4.0
    end
  end
end